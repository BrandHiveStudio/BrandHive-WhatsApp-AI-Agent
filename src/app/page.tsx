...
export default function Dashboard() {
  ...
  // Uses the shared cookie-based auth client (not a bare createClient())
  // so this connection is authenticated and Realtime's RLS-gated
  // postgres_changes events are actually delivered to it.
        ...
        <div className="px-5 py-4 border-b border-white/[0.06]">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-sm font-semibold text-white leading-tight">BrandHive Studio</h1>
              <p className="text-xs text-white/40 leading-tight mt-0.5">{conversations.length} conversation{conversations.length !== 1 ? "s" : ""}</p>
            </div>
            <button
              onClick={handleSignOut}
              className="text-[10px] text-white/30 hover:text-white/60 px-2 py-1 rounded-md transition-colors flex-shrink-0"
              title="Sign out"
            >
              Sign out
            </button>
